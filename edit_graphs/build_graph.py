...
def extract_node_captions(args):
    ...
    chat_args.model_path = "/home/user/Documents/ZJ/LLaVA-7B-v0"
...
def refine_node_captions(args):
    ...
    for _i in trange(len(captions)):
        ...
        API_KEY = 'sk-BkSGN3TQs9Tb6n7L5e611498007248E78d0e9dD903D1Ed81'
        API_HOST = 'https://api.key77qiqi.cn/v1'
...
def build_scenegraph(args):
    ...
    for i in range(num_segments):
        for j in range(i + 1, num_segments):
            if i == j:
                continue
            if bbox_overlaps[i, j] > 0.01 or bbox_overlaps[j, i] > 0.01:
            # if bbox_overlaps[i, j] > 0.01:
                weight = min(bbox_overlaps[i, j],bbox_overlaps[j, i])
    ...
    if len(labels) != 0:
        ...
        if not (Path(args.cachedir) / "cfslam_object_relations_new.json").exists():
        # if (Path(args.cachedir) / "cfslam_object_relations.json").exists():
            relation_queries = []
            for componentidx, component in enumerate(components):
                ...
                for u, v in zip(
                    ...
                    from edit_graphs.compute_position_relation import analyze_position
                    
                    # If the output is a valid JSON, then add it to the output dictionary
                    output_dict["object_relation"] = analyze_position(np.round(_bbox1.center, 1), np.round(_bbox1.extent, 1), np.round(_bbox2.center, 1), np.round(_bbox2.extent, 1))
            ...
            with open(Path(args.cachedir) / "cfslam_object_relations_new.json", "w") as f:
